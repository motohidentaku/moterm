#!/usr/bin/env bash
# Stage 5 E2E: NEO-UI で接続後、キーボードでコマンドを送り、neo 端末ペインに
# 出力が表示されることをスクショで確認（実端末描画＋キー入力ルーティングの立証）。
set -eu
cd "$(dirname "$0")/.."
IMAGE=moterm-dev
...
EOF
  ...
  xdotool mousemove --window "$WID" --sync 90 135 click 1
  sleep 3
  # TOFU モーダルを y で承認
  xdotool windowfocus "$WID" 2>/dev/null || true
  xdotool key --window "$WID" --clearmodifiers y
  sleep 4
  # キーボードでコマンド送信（neo→terminal_key→PTY）
  xdotool type --window "$WID" --clearmodifiers "echo NEO_TERM_OK_$((6*7))"
  xdotool key --window "$WID" Return
  sleep 3
  import -window root /work/artifacts/neo-terminal.png 2>/dev/null \
    || (xwd -root -silent | convert xwd:- /work/artifacts/neo-terminal.png)
