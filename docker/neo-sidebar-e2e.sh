#!/usr/bin/env bash
# Stage 4 E2E: NEO-UI サイドバーのホスト行をマウスクリック → 実 sshd へ接続 →
# シェルで touch したマーカーで合否判定。neo の入力ルーティング＋connect配線を立証。
set -eu
cd "$(dirname "$0")/.."
IMAGE=moterm-dev
...
EOF
  ...
  sleep 4
  import -window root /work/artifacts/neo-sidebar-after.png 2>/dev/null \
    || (xwd -root -silent | convert xwd:- /work/artifacts/neo-sidebar-after.png)
