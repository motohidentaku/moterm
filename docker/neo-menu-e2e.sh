#!/usr/bin/env bash
# neo タブ右クリックメニュー確認: 接続→タブを右クリック→ホバー→スクショ。
set -eu
cd "$(dirname "$0")/.."
IMAGE=moterm-dev
...
EOF
  ...
  sleep 4
  # タブ帯の local-demo タブを右クリック（タブは中央上部 y~65 付近、x~380）
  xdotool mousemove --sync 380 65 click 3
  sleep 1
  # メニューの2項目目あたりにホバー（ハイライト確認）
  xdotool mousemove --sync 360 120
  sleep 1
  import -window root /work/artifacts/neo-menu.png 2>/dev/null \
    || (xwd -root -silent | convert xwd:- /work/artifacts/neo-menu.png)
