#!/usr/bin/env bash
# neo テキスト選択＆スクロールバー確認: 接続→seq出力→端末上をドラッグ選択→スクショ。
set -eu
cd "$(dirname "$0")/.."
IMAGE=moterm-dev
...
EOF
  ...
  sleep 4
  # スクロールバック生成
  xdotool type --window "$WID" --clearmodifiers "seq 1 60"
  xdotool key --window "$WID" Return; sleep 3
  # 端末上をドラッグして数行選択（左ボタン押下→移動→解放）
  xdotool mousemove --sync 400 150 mousedown 1
  for yy in 160 190 220 250; do xdotool mousemove --sync 640 $yy; done
  xdotool mouseup 1
  sleep 2
  import -window root /work/artifacts/neo-select.png 2>/dev/null \
    || (xwd -root -silent | convert xwd:- /work/artifacts/neo-select.png)
